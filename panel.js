...
function getMonth(code) {
  ...
  return months[parseInt(code) - 1]
}

function setDefault() {
  document.getElementById("creator_name").textContent = "-"
  document.getElementById('creator_image').src = "creator_images/guest-user.jpg"
  document.getElementById('creator_subs').textContent = "-"
  document.getElementById('creator_views').textContent = "-"
  document.getElementById('creator_country').textContent = "-"
  document.getElementById('creator_joined').textContent = "-"
  document.getElementById('creator_banner').src = "assets/img/theme/img-1-1000x600.jpg"
}
  ...
  browser.tabs.query({windowId: myWindowId, active: true})
    .then((tabs) => {
      ...
      .then(response => response.json()).then((data) => {
        ...
        fetch(`https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&id=${channel_id}&key=${api}`)
        .then(response => response.json()).then((data) => {
          console.log(data)
          ...
          let country = data['items'][0]['snippet']['country']
          let publish_date = data['items'][0]['snippet']['publishedAt']
          let country_name = getCountryName(country)
          ...
          let viewst = ""
          if (subcount < 1000) {
            subst = subcount.toString()
          }
          else if (subcount < 1000000) {
            subst = (subcount / 1000).toFixed(1) + "K"
          } 
          else {
            subst = (subcount / 1000000).toFixed(1) + "M"
          }

          if (viewcount < 1000) {
            viewst = viewcount.toString()
          }
          else if (viewcount < 1000000) {
            viewst = (viewcount / 1000).toFixed(1) + "K"
          } 
          else if (viewcount < 1000000000) {
            viewst = (viewcount / 1000000).toFixed(1) + "M"
          }
          else {
            viewst = (viewcount / 1000000000).toFixed(1) + "B"
          }

          publish_string = publish_date
          let publish_year = publish_date.substring(0, 4)

          let publish_month = getMonth(publish_date.substring(5, 7))

          publish_string = publish_month + " " + publish_year 
          console.log(url)
          ...
          .then(response => response.json()).then((data) => {
              ...
              document.getElementById('creator_subs').textContent = subst
              document.getElementById('creator_views').textContent = viewst
              document.getElementById('creator_country').textContent = country_name
              document.getElementById('creator_joined').textContent = publish_string
